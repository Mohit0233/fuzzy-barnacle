...
resource "aws_instance" "github_runners" {
  ...
  user_data = base64encode(join("\n", [
    ...
    "export RUNNER_CFG_PAT=\"${var.runner_token}\"",
    ...
    "./config.sh --url https://github.com/${var.github_repo} --token $RUNNER_CFG_PAT --unattended --name \"$RUNNER_NAME\" --labels \"$RUNNER_NAME\"",
